...
void
Cluster::make_category_tree_layout()
{
  ...
  double node_pos_x = 0.0;
  double node_pos_y = 0.0;

  for (unsigned i_node = 0; i_node != _nodes.size(); ++i_node)
  {
    Node* current_node = _nodes[i_node];

    if (current_node->getDegree() < 10)
    {
      // Do nothing??
    }
    else
    {
      ...
      std::vector<Edge*> out_edges = current_node->outgoingEdges;
      for (unsigned i_out; i_out != out_edges.size(); ++i_out)
      {
        Edge* out_edge = out_edges[i_out];
      }
    }


    // Set all nodes at outgoing edges
//
//    for (unsigned i_out; i_out != out_edges.size(); ++i_out)
//    {
//      std::cout << "2" << std::endl;
//      Edge* out_edge = out_edges[i_out];
//      Node* out_node = out_edge->getTarget();
//
//      out_node->_x = ((double) rand() / (RAND_MAX + 1));
//      out_node->_y = ((double) rand() / (RAND_MAX + 1));
//
//      std::cout << out_node->_x << "   " << out_node->_y << std::endl;
//    }
    node_pos_x += 10000.0;
